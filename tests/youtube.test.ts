...
describe('youtube URL detection', () => {
  describe('extractYouTubeIds', () => {
    ...
    it('handles 11-character video IDs with hyphens and underscores', () => {
  ...
  describe('detectYouTubeUrls', () => {
    it('returns URL objects with id and original URL', () => {
      const urls = detectYouTubeUrls('Watch: https://youtube.com/watch?v=dQw4w9WgXcQ here');
      expect(urls).toEqual([{ id: 'dQw4w9WgXcQ', url: 'https://youtube.com/watch?v=dQw4w9WgXcQ' }]);
    });

    it('handles youtu.be URLs', () => {
      const urls = detectYouTubeUrls('See https://youtu.be/abc123DEF45');
      expect(urls).toEqual([{ id: 'abc123DEF45', url: 'https://youtu.be/abc123DEF45' }]);
    });
  });
});